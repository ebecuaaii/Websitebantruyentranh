...
function updateOrderSummary() {
  ...
  if (el("subtotalAmount"))
    el("subtotalAmount").textContent = subtotal.toLocaleString("vi-VN") + " đ";
  if (el("discountAmount"))
    el("discountAmount").textContent =
      "-" + discount.toLocaleString("vi-VN") + " đ";
  if (el("totalAmount"))
    el("totalAmount").textContent = total.toLocaleString("vi-VN") + " đ";
}
...
async function applyCoupon() {
  ...
  try {
    ...
    const res = await fetch(
      `${BASE_URL}/coupons/${code}?orderAmount=${subtotal}`,
      { headers: authHeaders() },
    );
    const json = await res.json();
    ...
    appliedCoupon = null;
    if (msgEl) {
      msgEl.textContent = e.message || "Mã không hợp lệ";
...
async function placeOrder(e) {
  ...
  const shippingAddress = form
    .querySelector('[name="shippingAddress"]')
    ?.value?.trim();
  ...
  const ward =
    document.querySelectorAll("select")[2]?.options[
      document.querySelectorAll("select")[2]?.selectedIndex
    ]?.text || "";
