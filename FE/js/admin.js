...
const adminRequest = async (path, options = {}) => {
  ...
  // Kiểm tra 401 Unauthorized - redirect to login
  if (response.status === 401) {
    console.warn(
      "Unauthorized (401). Session expired. Redirecting to login...",
    );
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const loadCategorySelect = async () => {
    ...
    try {
      ...
      console.error("Không nạp được danh mục:", err);
    }
  };
    ...
    fileInputs.forEach((input) => {
      input.addEventListener("change", function (e) {
        ...
        reader.onload = function (event) {
          ...
          const container =
            input.closest(".form-group") || input.closest(".image-upload");
  ...
  const setupMultipleImageUpload = () => {
    ...
    multiPickers.forEach((picker) => {
      picker.addEventListener("change", function (e) {
        const gallery = picker
          .closest(".form-group")
          .querySelector(".image-gallery-grid");
        if (!gallery) return;
  ...
  if (currentPath.includes("admin-orders.html")) {
    ...
    const renderTable = (page) => {
      ...
      if (pageOrders.length === 0) {
        tbody.innerHTML =
          '<tr><td colspan="7" style="text-align:center">Không tìm thấy đơn hàng nào.</td></tr>';
        return;
      }
      ...
      // Bind Actions
      document
        .querySelectorAll(".btn-view-o")
        .forEach(
          (btn) => (btn.onclick = () => viewOrderDetail(btn.dataset.id)),
        );
      document
        .querySelectorAll(".btn-edit-o")
        .forEach(
          (btn) =>
            (btn.onclick = () =>
              showStatusUpdate(btn.dataset.id, btn.dataset.status)),
        );
    ...
    const loadOrders = async () => {
      ...
      try {
        ...
        if (tbody) {
          tbody.innerHTML = `<tr><td colspan="100%" style="padding:20px;text-align:center;color:red;"><strong>❌ Lỗi:</strong> ${err.message}</td></tr>`;
        }
      }
    };
    ...
    const showStatusUpdate = (id, currentStatus) => {
      const modal =
        document.getElementById("statusModal") || createStatusModal();
      ...
      modal.querySelector("#btnCancelStatus").onclick = () =>
        (modal.style.display = "none");
    };
    ...
    const createStatusModal = () => {
      ...
      modal.style =
        "display:none;position:fixed;inset:0;background:rgba(0,0,0,.4);z-index:9999;align-items:center;justify-content:center;";
      modal.innerHTML = `
      ...
      try {
        ...
        document.querySelectorAll(".btn-del-coupon").forEach((btn) => {
          btn.onclick = async () => {
            ...
            try {
              await adminRequest(`/api/admin/coupons/${btn.dataset.id}`, {
                method: "DELETE",
              });
              loadCoupons();
            } catch (err) {
    ...
    if (couponForm) {
      couponForm.addEventListener("submit", async (e) => {
        ...
        const payload = {
          ...
          discountPercent:
            parseFloat(
              couponForm.querySelector('[name="discountPercent"]')?.value,
            ) || null,
          discountAmount:
            parseFloat(
              couponForm.querySelector('[name="discountAmount"]')?.value,
            ) || null,
          minOrderAmount:
            parseFloat(
              couponForm.querySelector('[name="minOrderAmount"]')?.value,
            ) || null,
          maxUsage:
            parseInt(couponForm.querySelector('[name="maxUsage"]')?.value) ||
            null,
          active: couponForm.querySelector('[name="active"]')?.checked ?? true,
          expiresAt:
            couponForm.querySelector('[name="expiresAt"]')?.value || null,
        };
        try {
          if (id)
  ...
  if (currentPath.includes("admin-products.html")) {
      ...
      document.querySelectorAll(".btn-del-p").forEach((btn) => {
        btn.onclick = async () => {
          if (confirm(`Xóa sản phẩm này?`)) {
            try {
              await adminRequest(`/api/admin/products/${btn.dataset.id}`, {
                method: "DELETE",
              });
    ...
    const loadProducts = async () => {
      ...
      try {
        ...
        console.error("Lỗi danh sách SP:", err);
        if (tbody) {
          tbody.innerHTML = `<tr><td colspan="100%" style="padding:20px;text-align:center;color:red;"><strong>❌ Lỗi:</strong> ${err.message}</td></tr>`;
        }
      }
        ...
        document.querySelectorAll(".btn-del-c").forEach((btn) => {
          btn.onclick = async () => {
            if (confirm(`Xóa danh mục ID: ${btn.dataset.id}?`)) {
              try {
                await adminRequest(`/api/admin/categories/${btn.dataset.id}`, {
                  method: "DELETE",
                });
                alert("Đã xóa!");
                loadCategories();
  ...
  if (currentPath.includes("admin-users.html")) {
      ...
      document.querySelectorAll(".btn-del-u").forEach((btn) => {
        btn.onclick = async () => {
          if (confirm(`Xóa tài khoản ID: ${btn.dataset.id}?`)) {
            try {
              await adminRequest(`/api/admin/users/${btn.dataset.id}`, {
                method: "DELETE",
              });
              alert("Đã xóa!");
              loadUsers();
    ...
    document
      ...
      ?.addEventListener("input", () => {
        adminRequest("/api/admin/users").then((data) => {
          allUsers = data;
          applyUserFilters();
        });
      });
          ...
          Object.keys(data).forEach((key) => {
            ...
            if (input) {
              if (input.type === "checkbox") input.checked = data[key];
                ...
                input.value = ""; // Don't show password
