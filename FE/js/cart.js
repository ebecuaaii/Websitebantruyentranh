...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const summaryTamTinh = document.querySelector(
  ...
  const renderCart = (cartData) => {
    ...
    if (summaryTamTinh)
      summaryTamTinh.textContent = total.toLocaleString("vi-VN") + " đ";
  };
  ...
  const loadCart = async () => {
    try {
      ...
      if (cartItemsWrapper) {
        cartItemsWrapper.innerHTML =
          '<p class="error">Không thể tải giỏ hàng. Vui lòng đăng nhập.</p>';
      }
    }
  };
  ...
  if (getToken()) {
    ...
    if (cartItemsWrapper) {
      cartItemsWrapper.innerHTML =
        '<p class="error">Vui lòng <a href="login.html">đăng nhập</a> để xem giỏ hàng.</p>';
    }
  }
});
