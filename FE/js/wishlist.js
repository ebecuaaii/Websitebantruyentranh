...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const renderWishlist = (products) => {
    ...
    if (products.length === 0) {
      wishlistContainer.innerHTML =
        '<p class="no-data">Danh sách yêu thích của bạn đang trống.</p>';
      return;
    }
  ...
  const loadWishlist = async () => {
    try {
      ...
      if (wishlistContainer) {
        wishlistContainer.innerHTML =
          '<p class="error">Không thể tải danh sách yêu thích. Vui lòng đăng nhập.</p>';
      }
    }
  };
  ...
  if (getToken()) {
    ...
    if (wishlistContainer) {
      wishlistContainer.innerHTML =
        '<p class="error">Vui lòng <a href="login.html">đăng nhập</a> để xem danh sách yêu thích.</p>';
    }
  }
});
