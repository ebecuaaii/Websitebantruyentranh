...
async function loadProductDetail() {
  ...
  try {
    ...
    if (el("productPrice"))
      el("productPrice").textContent =
        (p.price || 0).toLocaleString("vi-VN") + " đ";
    if (el("productDesc")) el("productDesc").textContent = p.description || "";
    if (el("productImage"))
      el("productImage").src = p.imageUrl || "https://placehold.co/300x400";
    if (el("productStock"))
      el("productStock").textContent = p.stock > 0 ? "Còn hàng" : "Hết hàng";
  } catch (e) {
    console.error("Lỗi load product:", e);
  }
}
...
async function submitReview(e) {
  ...
  try {
    ...
    alert("Lỗi gửi đánh giá: " + e.message);
  }
}
